...
static bool IRAM_ATTR captureCallbackFunction(mcpwm_unit_t mcpwm,
...
class EventTimerClass {
 public:
  ...
  bool begin(int pin, EventHandler userHandler) {
    ...
    mcpwm_gpio_init(MCPWM_UNIT_0, MCPWM_CAP_0, pin);
    mcpwm_gpio_init(MCPWM_UNIT_0, MCPWM_CAP_1, pin);
  ...
  inline unsigned long IRAM_ATTR elapsedTicksSinceLastEvent() { return (micros() - thisEventMicros) * TICKSPERMICROSEC; };
  ...
  inline unsigned int ticksPerMicrosec() { return TICKSPERMICROSEC; };
